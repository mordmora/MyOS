...
#include "debug.h"
gdt_t gdt;
gdt_descriptor_t entries[3];

void init_table_segment(unsigned int i, unsigned int base, unsigned int limit, unsigned char access, unsigned char gran){
...
void print_gdt_layout(uint32_t i){
        kprintf("gdt_ptr:\n\tbase_low: %p\n\tbase_mid: %p\n\tbase_high: %p\n\tlimit: %p\n\tflimits: %p\n\taccess: %p\n",
            entries[i].base_l, entries[i].base_mid, entries[i].base_high, entries[i].limit, entries[i].flimits, entries[i].access);
